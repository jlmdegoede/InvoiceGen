...
class CompanyTable(tables.Table):
    ...
    recent_products = tables.TemplateColumn('{% for product in record.recent_products %} <a href="{% url "view_product" product.id product.title|slugify %}">{{ product.title }}</a> {% endfor %}', verbose_name='Recente opdrachten')
